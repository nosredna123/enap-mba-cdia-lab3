...
7. Junta todas as planilhas processadas em `output/classificacoes_relatorios.csv` (por padrão) e salva o cache em `cache/classificacao_cache.json`.
...
python process_relatorios.py \
  ...
  --output-csv output/classificacoes_relatorios.csv \
...
- `--output-csv`: caminho do CSV combinado de saída (será criado com diretórios pai).
...
- A resposta do modelo deve conter `categoria_escolhida` e `categorias_atualizadas` (com nome, descrição e exemplos reais fornecidos), sempre em português do Brasil.
- As classificações e categorias retornadas são armazenadas no cache; exemplos são deduplicados e mantêm no máximo três amostras por categoria para próximos prompts.
- Caso o modelo proponha nova categoria, o exemplo inicial é o próprio texto classificado antes de ser salvo em cache.

## Estrutura de arquivos
- `process_relatorios.py`: CLI principal que lê os `.xls`, valida colunas, chama o LLM com cache e monta o CSV final.
...
- `output/` e `cache/`: criados automaticamente para o CSV combinado e o cache JSON, respectivamente.
